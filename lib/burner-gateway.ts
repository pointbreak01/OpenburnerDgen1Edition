...
export async function signTransactionWithGateway(
  ...
  try {
    console.log("\n🔐 [Gateway] signTransactionWithGateway CALLED");
    console.log("📋 [Gateway] Transaction object received:");
    ...
    // CRITICAL: Ensure data is present
    if (!transaction.data || transaction.data === "0x" || transaction.data === "") {
      console.warn("⚠️ [Gateway] WARNING: transaction.data is empty or missing!");
      console.warn("   This will result in an empty transaction!");
    }
    ...
    console.log("\n✅ [Gateway] Transaction object created");
