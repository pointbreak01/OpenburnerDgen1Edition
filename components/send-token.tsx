...
import { CheckCircle, ExternalLink, Clock, X, Wallet } from "lucide-react";
...
export function SendToken({
  ...
  const { address, rpcUrl, chainId, keySlot, chainName, activeWalletType, activeSmartWalletAddress } = useWalletStore();
  ...
  const [currentBalance, setCurrentBalance] = useState(token.balance);
  ...
  // Reload balance when wallet changes
  useEffect(() => {
    async function reloadBalance() {
      const walletToCheck = isSmartWallet && smartWalletAddress ? smartWalletAddress : address;
      if (!walletToCheck || !rpcUrl) return;

      try {
        const provider = new ethers.JsonRpcProvider(rpcUrl);
        let balance = "0";

        if (token.address === "ETH" || token.address === "native") {
          // Native token
          const balanceWei = await provider.getBalance(walletToCheck);
          balance = ethers.formatEther(balanceWei);
        } else if (ethers.isAddress(token.address)) {
          // ERC-20 token - validate address first
          const contract = new ethers.Contract(token.address, ERC20_ABI, provider);
          const balanceRaw = await contract.balanceOf(walletToCheck);
          balance = ethers.formatUnits(balanceRaw, token.decimals);
        } else {
        ...
        console.log(`💰 [SendToken] Updated balance for ${token.symbol}: ${balance}`);
    ...
    reloadBalance();
  }, [activeWalletType, activeSmartWalletAddress, token.address, token.symbol, address, rpcUrl]);
  ...
  async function handlePinSubmit(enteredPin: string) {
    ...
    try {
      ...
      if (isSmartWallet && smartWalletAddress) {
        // SMART WALLET MODE: Create transaction through Smart Wallet contract
        console.log("\n═══════════════════════════════════════════════════════");
        console.log("💼 [SendToken] SMART WALLET MODE ATTIVO");
        console.log("═══════════════════════════════════════════════════════");
        console.log(`   Smart Wallet Contract: ${smartWalletAddress}`);
        console.log(`   Owner (Burner Card): ${address}`);
        console.log(`   Recipient: ${recipient}`);
        console.log(`   Amount: ${amount} ${token.symbol}`);
        console.log(`   Token Address: ${token.address}`);
        
        // Create the call data for the smart wallet
        let call;
        if (token.address === "native") {
          // Native ETH transfer through smart wallet
          const amountWei = ethers.parseEther(amount);
          call = createSendETHCall(recipient, amountWei);
        } else {
          // ERC20 token transfer through smart wallet
          const amountWei = ethers.parseUnits(amount, token.decimals);
          call = createSendERC20Call(token.address, recipient, amountWei);
        }
        
        // Create the transaction that calls the smart wallet's execute function
        // This transaction will be signed by the owner (Burner card)
        transaction = await createSmartWalletTransaction(
          provider,
          smartWalletAddress,
          address,
          call,
        ...
        console.log("✅ [SendToken] Smart Wallet transaction created");
        console.log("📋 [SendToken] Transaction details:");
        console.log("   to:", transaction.to);
        console.log("   value:", transaction.value);
        console.log("   data:", transaction.data);
        console.log("   data type:", typeof transaction.data);
        console.log("   data length:", transaction.data ? (transaction.data as string).length : 0);
        console.log("   nonce:", transaction.nonce);
        console.log("   gasLimit:", transaction.gasLimit);
        
        if (!transaction.data || transaction.data === "0x" || transaction.data === "") {
          console.error("❌❌❌ [SendToken] CRITICAL ERROR: transaction.data is empty!");
          console.error("   This means createSmartWalletTransaction did not encode the call data properly");
          throw new Error("Failed to create Smart Wallet transaction: empty call data");
        }
      } else {
        // NORMAL MODE: Direct transfer from Burner card address
        console.log("💳 [SendToken] Using direct wallet mode");
