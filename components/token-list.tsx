...
export function TokenList({ 
  ...
  const { address, rpcUrl, chainId, activeWalletType, activeSmartWalletAddress } = useWalletStore();
  ...
  useEffect(() => {
    // Use Smart Wallet address if configured, otherwise use owner address
    const walletToCheck = isSmartWallet && smartWalletAddress ? smartWalletAddress : address;
    
    console.log(`🔄 [Token List] Wallet changed - reloading tokens for ${isSmartWallet ? 'Smart Wallet' : 'Burner card'}: ${walletToCheck}`);
    
    // Check cache first (no expiration)
    const cacheKey = `${chainId}_${walletToCheck}`;
    const cached = tokenCache.get(cacheKey);
    
    if (cached) {
      console.log("📦 Using cached token data for", isSmartWallet ? "Smart Wallet" : "Burner card");
      setTokens(cached);
  ...
  }, [address, rpcUrl, chainId, activeWalletType, activeSmartWalletAddress]);
  ...
  async function loadTokens() {
    ...
    try {
      ...
      console.log(`💼 Checking balances for: ${isSmartWallet && smartWalletAddress ? 'Smart Wallet' : 'Burner card'}`);
      console.log(`   Address: ${walletToCheck}`);
      ...
      try {
        const nativeBalance = await provider.getBalance(walletToCheck);
      ...
      const balances = await batchGetBalances(provider, allAddresses, walletToCheck);
      ...
      // For custom tokens, use saved metadata or fetch if needed
      if (customTokensToCheck.length > 0) {
        console.log(`🔍 Processing ${customTokensToCheck.length} custom tokens...`);
        const storedCustomTokens = getStoredCustomTokens();
        const customTokenMetadataMap = new Map(
          storedCustomTokens.map(t => [t.address.toLowerCase(), t])
        );
        
        // Check which ones need metadata fetched
        const tokensNeedingMetadata = customTokensToCheck.filter(
          addr => !customTokenMetadataMap.has(addr.toLowerCase())
        );
        
        // Fetch metadata only for tokens without saved metadata
        let metadata = new Map();
        if (tokensNeedingMetadata.length > 0) {
          console.log(`🔍 Fetching metadata for ${tokensNeedingMetadata.length} custom tokens...`);
          metadata = await batchGetTokenMetadata(provider, tokensNeedingMetadata);
        }
        
        for (const tokenAddr of customTokensToCheck) {
          const balance = balances.get(tokenAddr.toLowerCase());
          
          // Try saved metadata first, then fetched metadata
          let meta = customTokenMetadataMap.get(tokenAddr.toLowerCase());
          if (!meta) {
            const fetchedMeta = metadata.get(tokenAddr.toLowerCase());
            if (fetchedMeta && fetchedMeta.success) {
              meta = {
                address: tokenAddr,
  ...
  function getStoredTokenAddresses(): string[] {
    ...
    const newKey = `customTokens_${chainId}`;
    const newStored = localStorage.getItem(newKey);
    if (newStored) {
      try {
        const customTokens = JSON.parse(newStored);
        return customTokens.map((t: any) => t.address);
      } catch (e) {
        console.error("Error parsing custom tokens:", e);
      }
    }
    
    // Fallback to old format (addresses only)
    const key = `tokens_${chainId}`;
    const stored = localStorage.getItem(key);
    return stored ? JSON.parse(stored) : [];
  }
  
  function getStoredCustomTokens(): Array<{address: string, symbol: string, name: string, decimals: number}> {
    const key = `customTokens_${chainId}`;
    const stored = localStorage.getItem(key);
    if (stored) {
      try {
  ...
  async function handleAddToken() {
    ...
    try {
      ...
      const [balance, symbol, name, decimals] = await Promise.all([
        contract.balanceOf(walletToCheck),
        contract.symbol(),
        contract.name(),
        contract.decimals(),
      ]);
      
      console.log(`💼 Checking token balance for: ${isSmartWallet && smartWalletAddress ? 'Smart Wallet' : 'Burner card'}`);
  ...
  async function handleManualRefresh() {
    ...
    console.log(`💼 Refreshing for: ${isSmartWallet && smartWalletAddress ? 'Smart Wallet' : 'Burner card'}`);
      ...
      <div className="mb-3 px-3 py-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
        <p className="text-xs font-medium text-blue-900 dark:text-blue-100">
          {isSmartWallet && smartWalletAddress ? (
            <>
              💼 Showing <strong>Smart Wallet</strong> balances ({smartWalletAddress.slice(0, 6)}...{smartWalletAddress.slice(-4)})
            </>
          ) : (
            <>
              💳 Showing <strong>Burner Card</strong> balances ({address?.slice(0, 6)}...{address?.slice(-4)})
            </>
          )}
        </p>
      </div>
